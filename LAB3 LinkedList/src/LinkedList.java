...
public class LinkedList {
    ...
    private LLnode DeleteLast(LLnode head) {
        ...
        if (head.getNext() == null) {
            ...
            System.out.println(head.getData() + " is the only node in the list which has been deleted");
            return null;
        } else {
            LLnode hp = head;
            while (hp.getNext().getNext() != null) {
                hp = hp.getNext();
            }
            System.out.println(">    Deleting last node in the linked list...");
            System.out.println("Last node " + hp.getNext().getData() + " has been deleted successfully");
            hp.setNext(null);
        }
        return head;
    }
    ...
    private void Odd2Even2Odd(LLnode head) {
        LLnode hp = head;
        if (hp.getNext() == null) {
            hp.setData((hp.getData() % 2 == 0) ? hp.getData() + 1 : hp.getData() - 1);
        } else {
            while (hp != null) {
                hp.setData((hp.getData() % 2 == 0) ? hp.getData() + 1 : hp.getData() - 1);
                hp = hp.getNext();
